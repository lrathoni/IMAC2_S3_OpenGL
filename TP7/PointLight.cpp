...
int main(int argc, char** argv) {
    ...
    uLightPos_vs = glGetUniformLocation(SceneProgram.m_Program.getGLId(), "uLightPos_vs");
    ...
    while(!done) {
        ...
        glUniform3fv(uLightPos_vs, 1, value_ptr(vec3(camera.getViewMatrix()*vec4(LightDir,0))));
        glUniform3fv(uLightIntensity, 1, value_ptr(LightIntensity));
    
        //turn light
        //LightDir = rotateX(vec3(camera.getViewMatrix()*vec4(LightDir,0)), windowManager.getTime());
